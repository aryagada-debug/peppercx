...
DECLARE
  v_names text;
BEGIN
  IF _role_key NOT IN ('principal_bopm','senior_bopm','bopm') THEN
    RETURN;
  END IF;
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    IF OLD.role_key IN ('principal_bopm','senior_bopm','bopm') THEN
      PERFORM public._recompute_deal_bopm_field(OLD.deal_id, OLD.role_key);
    END IF;
    RETURN OLD;
  END IF;

  IF NEW.role_key IN ('principal_bopm','senior_bopm','bopm') THEN
    PERFORM public._recompute_deal_bopm_field(NEW.deal_id, NEW.role_key);
  END IF;

  IF TG_OP = 'UPDATE' AND (OLD.deal_id <> NEW.deal_id OR OLD.role_key <> NEW.role_key)
     AND OLD.role_key IN ('principal_bopm','senior_bopm','bopm') THEN
    PERFORM public._recompute_deal_bopm_field(OLD.deal_id, OLD.role_key);
  END IF;
...
CREATE TRIGGER trg_sync_bopm_fields
...
-- One-time backfill: for every (deal, bopm-role) pair that has assignments,
-- write the joined names back into the deal sheet text fields. Existing
-- orphan text on deals without any assignment is left alone so the user
-- can review/clear them via the UI.
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    SELECT DISTINCT deal_id, role_key
    FROM public.staffing_assignments
    WHERE role_key IN ('principal_bopm','senior_bopm','bopm')
  LOOP
